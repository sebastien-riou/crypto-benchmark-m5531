...
#define SCLIB_ERR_ATR_UNRECOGNIZED              0x00001001  ///< Unrecognized ATR \hideinitializer
...
/**
  * @brief Process card detect event by checking variable.
  * @param[in] num                  Smartcard interface number. From 0 ~ ( \ref SC_INTERFACE_NUM - 1)
  * @param[in] pu32CardIsInserted    pointer points to a variable which indicates the card detection status.
  * @return Card detect event occur or not
  * @retval 1 Card detect event occurred
  * @retval 0 Card detect event did not occur
  * @note For MCU without H/W card detection pin, we can use a GPIO to implement the S/W card detection.
  *       So we need a variable for internal control in this API.
  *       The API is designed for working with GPIO interrupt. It must be called in GPIO IRQ handler.
  */
uint32_t SCLIB_CheckCDEvent_ByVar(uint32_t num, volatile uint32_t *pu32CardIsInserted);
