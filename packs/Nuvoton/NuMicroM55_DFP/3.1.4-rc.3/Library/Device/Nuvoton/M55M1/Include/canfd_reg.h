...
typedef struct
{
     ...
     * |        |          |1 = Loop Back Mode is enabled (refer to 1.1.5.1 TEST Mode).
     ...
     * |        |          |1 = The CPU has write access to the protected configuration registers (while INIT (CANFD_CCCR[0]) = 1).
     ...
     * |        |          |If this bit is set, the CAN FD controller uses the CAN FD frame format as specified by the Bosch CAN FD Specification V1.0.
     ...
     * |        |          |01 = Timestamp counter value incremented according to TCP and the time unit is CAN bit times.
     * |        |          |10 = Timestamp counter value incremented according to TCP and the time unit is CAN Core clock.
     * |        |          |11 = Same as '00'.
     ...
     * |        |          |When TSS (CANFD_TSCC[[1:0]) = 2'b01, the Timestamp Counter is incremented in multiples of CAN bit times [ 1...16 ] depending on the configuration of TCP (CANFD_TSCC[19:16])
     * |        |          |A wrap around sets interrupt flag TSW (CANFD_IR[16])
     * |        |          |Write access resets the counter to zero
     * |        |          |When TSS (CANFD_TSCC[1:0]) = 10, TSC reflects the external Timestamp Counter value
     ...
     * |        |          |When operating in Continuous mode, a write to CANFD_TOCV presets the counter to the value configured by TOP (CANFD_TOCC[31:16]) and continues down-counting
     * |        |          |When the Timeout Counter is controlled by one of the FIFOs, an empty FIFO presets the counter to the value configured by TOP (CANFD_TOCC[31:16])
     ...
     * |        |          |When decremented to zero, interrupt flag TOO (CANFD_IR[18]) is set and the timeout counter is stopped
     ...
     * |        |          |101 = Bit0Error : During the transmission of a message (or acknowledge bit, or active errorflag, or overloadflag), the device wanted to send a dominant level (data or identifier bit logical value 0), but the monitored bus value was recessive
     ...
     * |        |          |It normally takes part in bus communication and sends an active errorflag when an error has been detected.
     ...
     * |        |          |Type of last error that occurred in the data phase of a CAN FD format frame with its BRSflag set
     * |        |          |Coding is the same as for LEC
     * |        |          |This field will be cleared to zero when a CAN FD format frame with its BRSflag set has been transferred (reception or transmission) without error.
     * |[11]    |RESI      |ESIflag of last received CAN FD Message
     * |        |          |This bit is set together with RFDF, independent of acceptance filtering.
     * |        |          |0 = Last received CAN FD message did not have its ESIflag set.
     * |        |          |1 = Last received CAN FD message had its ESIflag set.
     * |[12]    |RBRS      |BRSflag of last received CAN FD Message
     * |        |          |This bit is set together with RFDF, independent of acceptance filtering.
     * |        |          |0 = Last received CAN FD message did not have its BRSflag set.
     * |        |          |1 = Last received CAN FD message had its BRSflag set.
     ...
     * |        |          |1 = Message in CAN FD format with FDFflag set has been received.
     ...
     * |        |          |Position of the secondary sample point, defined by the sum of the measured delay from CANx_TXD to CANx_RXD and TDCO (CANFD_TDCR[[14:8])
     ...
     * |        |          |0 = Tx Event FIFO is not full.
     * |        |          |1 = Tx Event FIFO is full.
     ...
     * |        |          |Theflag is set, when the Rx Handler
     ...
     * |        |          |the New Dataflag for a dedicated Rx Buffer is not set, a partly stored message is overwritten when the next message is stored to this location.
     ...
     * |        |          |To leave Restricted Operation Mode, the Host CPU has to reset ASM (CANFD_CCCR[2]).
     ...
     * |        |          |Controlled by input signal m_can_aeim_berr[0] generated by an optional external parity / ECC logic attached to the Message RAM.
     ...
     * |        |          |Controlled by input signal m_can_aeim_berr[1] generated by an optional external parity / ECC logic attached to the Message RAM
     ...
     * |        |          |1 = Protocol error in arbitration phase detected (LEC (CANFD_PSR[2:0]) no equal 0 or 7).
     ...
     * |        |          |Start address of standard Message ID filter list (32-bit word address, refer to Figure 1.1-11).
     ...
     * |        |          |Start address of extended Message ID filter list (32-bit word address, refer to Figure 1.1-11)
     ...
     * |        |          |Range is 0 to LSS (CANFD_SIDFC[23:16]) - 1 or LSE (CANFD_XIDFC[22:16]) - 1
     * |[15]    |FLST      |Filter List
     ...
     * |        |          |1-64 = Level for Rx FIFO 0 watermark interrupt (RF0W(CANFD_IR[1]).
     ...
     * |        |          |This bit is a copy of interrupt flag RF0L (CANFD_IR[3])
     ...
     * |        |          |Start address of Rx FIFO 1 in Message RAM (32-bit word address, refer to Figure 1.1-11).
     ...
     * |        |          |0= Watermark interrupt disabled.
     * |        |          |1-64 = Level for Rx FIFO 1 watermark interrupt (RF1W(CANFD_IR[5])).
     * |        |          |>64 = Watermark interrupt disabled.
     ...
     * |[13:8]  |F1GI      |Rx FIFO 1 Get Index
     * |        |          |Rx FIFO 1 read index pointer, range 0 to 63.
     * |[21:16] |F1PI      |Rx FIFO 1 Fill Level
     * |        |          |Number of elements stored in Rx FIFO 1, range 0 to 64.
     * |[24]    |F1F       |Rx FIFO 1 Full
     * |        |          |0 = Rx FIFO 1 is not full.
     * |        |          |1 = Rx FIFO 1 is full.
     * |[25]    |RF1L      |Rx FIFO 1 Message Lost
     * |        |          |This bit is a copy of interrupt flag RF1L(CANFD_IR[7])
     * |        |          |When RF1L(CANFD_IR[7]) is reset, this bit is also reset
     * |        |          |0= No Rx FIFO 1 message lost.
     * |        |          |1= Rx FIFO 1 message lost, also set after write attempt to Rx FIFO 1 of size zero.
     ...
     * |        |          |Start address of Tx Buffers section in Message RAM (32-bit word address, refer to Figure 1.1-11).
     ...
     * |        |          |Read as zero when Tx Queue operation is configured (TFQM (CANFD_TXBC[30]) = 1).
     ...
     * |        |          |110 = 48 byte data field.
     * |        |          |111 = 64 byte data field.
     * |        |          |Note: In case the data length code DLC of a Tx Buffer element is configured to a value higher than the Tx Buffer data field size TBDS(CANFD_TXESC[2:0]), the bytes not defined by the Tx Buffer are transmitted as 0xCC (padding bytes).
     * @var CANFD_T::TXBRP
     ...
     * |        |          |After a CANFD_TXBRP bit has been set, a Tx scan (refer to 1.1.5.5, Tx Handling) is started to check for the pending Tx request with the highest priority (Tx Buffer with lowest Message ID).
     ...
     * |        |          |Start address of Tx Event FIFO in Message RAM (32-bit word address, refer to Figure 1.1-11).
     ...
     * |[12:8]  |EFGI      |Event FIFO Get Index
     * |        |          |Tx Event FIFO read index pointer, range 0 to 31
     * |[20:16] |EFPI      |Event FIFO Put Index
     * |        |          |Tx Event FIFO write index pointer, range 0 to 31
     * |[24]    |EFF       |Event FIFO Full
     * |        |          |0= Tx Event FIFO is not full.
     * |        |          |1= Tx Event FIFO is full.
     * |[25]    |TEFL      |Tx Event FIFO Element Lost
     * |        |          |This bit is a copy of interrupt flag TEFL (CANFD_IR[15])
     * |        |          |When TEFL is reset, this bit is also reset.
     * |        |          |0= No Tx Event FIFO element lost.
     * |        |          |1= Tx Event FIFO element lost, also set after write attempt to Tx Event FIFO of size zero.
     * @var CANFD_T::TXEFA
    ...
    __I  uint32_t ECR;                   /*!< [0x0040] Error Counter Register                                           */
    __I  uint32_t PSR;                   /*!< [0x0044] Protocol Status Register                                         */
...
#define CANFD_IR_BEC_Pos                 (20)                                              /*!< CANFD_T::IR: BEC Position              */
