...
#define USBHn_EHCI_COM_AREA_SECTION_(x) __attribute__((section(x)))
    ...
    #error "The qh/qtd/itd/sitd area needs to be placed in the Non-Cacheable region,please.(.bss.NonCacheable.ZeroInit)"
#endif
    ...
    #define USBH_EHCI_MEM_QH_SIZE           (USBH_EHCI_MAX_PIPES * 512U)
#endif
#ifndef USBH_EHCI_MEM_QTD_SIZE
    #define USBH_EHCI_MEM_QTD_SIZE          (USBH_EHCI_MAX_PIPES * 512U)
#endif
...
static HSUSBH_T                    *usbh_ehci_reg_ptr[USBH_EHCI_TT_INSTANCES];
...
static uint16_t USBH_EHCI_PFL_GetSize(uint8_t ctrl)
{
    ...
    usbcmd = usbh_ehci_reg_ptr[ctrl]->UCMDR;
    sz  =   1024;
    i   = (usbcmd >> 2) & 0x03U;
...
static void USBH_EHCI_IOC(uint32_t *ptr_pfl_entry)
{
    ...
    if (ptr_pfl_entry == NULL)
    {
        return;
    }

    if ((*ptr_pfl_entry & 1U) != 0U)
    {
        return;    // If terminate bit is 1 (not a valid entry)
    }
...
static bool USBH_EHCI_StartStop(uint8_t ctrl, uint8_t type, bool start)
{
    ...
    if (start)
    {
        if ((usbh_ehci_reg_ptr[ctrl]->UCMDR & msk) == 0U)          // If schedule disabled
        {
            usbh_ehci_reg_ptr[ctrl]->UCMDR |=  msk;                  // enable schedule
        }
    }
    else
    {
        if ((usbh_ehci_reg_ptr[ctrl]->UCMDR & msk) != 0U)          // If schedule enabled
        {
            usbh_ehci_reg_ptr[ctrl]->UCMDR &= ~msk;                  // disable schedule
        }
    }
...
static bool USBH_EHCI_qTD_Enqueue(uint8_t ctrl, USBH_TransferInfo_t *ptr_TI, USBH_EHCI_qH *ptr_qH, USBH_EHCI_qTD *ptr_qTD, uint32_t packet, uint8_t *data, uint32_t num)
{
    ...
    else if (num <= 16536U)
    {
        ...
        if (sz > (1U * 4096U))
        {
            ptr_qTD->DW4.BufPtr = ptr_qTD->DW3.BufPtr + 1U;

            if (sz > (2U * 4096U))
            {
                ptr_qTD->DW5.BufPtr = ptr_qTD->DW3.BufPtr + 2U;

                if (sz > (3U * 4096U))
                {
                    ptr_qTD->DW6.BufPtr = ptr_qTD->DW3.BufPtr + 3U;

                    if (sz > (4U * 4096U))
                    {
                        ptr_qTD->DW7.BufPtr = ptr_qTD->DW3.BufPtr + 4U;
                    }
                }
            }
        }
    ...
    switch (((USBH_EHCI_COMMON *)ptr_pe)->DW0.Typ)
    {
        ...
        case USBH_EHCI_ELEMENT_TYPE_siTD:
            ...
            if (ptr_TI->active == 0U)
            {
                ...
                pfl_index = ((usbh_ehci_reg_ptr[ctrl]->UFINDR) >> 3) + 5U;
            }
            else
            {
...
static bool USBH_EHCI_qTD_Dequeue(uint8_t ctrl, USBH_EHCI_qH *ptr_qH, USBH_EHCI_qTD *ptr_qTD)
{
    ...
    if ((ptr_qH->DW6.Status & (1UL << 7)) != 0U)
    {
        ...
        while ((ptr_qH->DW6.Status & (1UL << 7)) != 0U)
        {
            __NOP();    // Wait for deactivate
        }
    }
...
static void USBH_EHCI_qH_ClearAll(uint8_t ctrl)
{
    ...
    ptr_qH = (USBH_EHCI_qH *)((uint32_t)(usbh_ehci_ptr[ctrl])->ptr_qH);
...
static USBH_EHCI_qH *USBH_EHCI_qH_GetPrevious(uint8_t ctrl, USBH_EHCI_qH *ptr_qH)
{
    ...
    ptr_prev_qH = (USBH_EHCI_qH *)((uint32_t)(usbh_ehci_ptr[ctrl])->ptr_qH);
    ...
    while (cnt != 0U)
    {
        ...
        if ((ptr_prev_qH->DW0.T == 0U) && (((uint32_t)ptr_prev_qH->DW0.HorizLinkPtr << 5) == (uint32_t)ptr_qH))
        {
            return ptr_prev_qH;
        }
...
static USBH_EHCI_qH *USBH_EHCI_qH_Unlink(USBH_EHCI_qH *ptr_first_qH, USBH_EHCI_qH *ptr_qH)
{
    ...
    if ((ptr_first_qH_ == NULL) || (ptr_qH_ == NULL))
    {
...
static bool USBH_EHCI_qH_SetIntEntries(uint8_t ctrl, USBH_EHCI_qH *ptr_qH, uint8_t ep_speed, uint8_t ep_interval)
{
    ...
    if (USBH_EHCI_qH_ClearIntEntries(ctrl, ptr_qH) == false)
    {
        return false;
    }
...
static USBH_TransferInfo_t *USBH_EHCI_TI_GetFree(uint8_t ctrl)
{
    ...
    ptr_TI = (usbh_ehci_ptr[ctrl])->ptr_TI;
...
static USBH_TransferInfo_t *USBH_EHCI_TI_Find_qTD(uint8_t ctrl, USBH_EHCI_qTD *ptr_qTD)
{
    ...
    while (cnt != 0U)
    {
        ...
        if (((uint32_t)ptr_TI->ptr_transfer & ~0x1FU) == (uint32_t)ptr_qTD)
        {
            return ptr_TI;
        }
...
static USBH_TransferInfo_t *USBH_EHCI_TI_Find_qH(uint8_t ctrl, USBH_EHCI_qH *ptr_qH)
{
    ...
    while (cnt != 0U)
    {
        ...
        if ((uint32_t)ptr_TI->ptr_pipe == (uint32_t)ptr_qH)
        {
            return ptr_TI;
        }
...
static USBH_TransferInfo_t *USBH_EHCI_TI_Find_PE(uint8_t ctrl, void *ptr_pe)
{
    ...
    while (cnt != 0U)
    {
        ...
        if ((uint32_t)ptr_TI->ptr_pipe      == (uint32_t)ptr_pe)
        {
            return ptr_TI;
        }

        if ((uint32_t)ptr_TI->iso_ptr_pipe1 == (uint32_t)ptr_pe)
        {
            return ptr_TI;
        }
...
*/
static ARM_DRIVER_VERSION USBH0_HW_GetVersion(void)
{
    return usbh_driver_version;
}
#if   (USBH_EHCI_TT_INSTANCES >= 2)
static ARM_DRIVER_VERSION USBH1_HW_GetVersion(void)
{
    return usbh_driver_version;
}
#endif
...
{
    uint32_t port_num = (usbh_ehci_reg_ptr[ctrl]->EHCSPR & 0xFU);
...
static ARM_USBH_CAPABILITIES USBH0_HW_GetCapabilities(void)
{
    return USBH_HW_GetCapabilities(0);
}
#if   (USBH_EHCI_TT_INSTANCES >= 2)
static ARM_USBH_CAPABILITIES USBH1_HW_GetCapabilities(void)
{
    return USBH_HW_GetCapabilities(1);
}
#endif
...
static int32_t USBH_HW_Initialize(uint8_t ctrl, ARM_USBH_SignalPortEvent_t cb_port_event, ARM_USBH_SignalPipeEvent_t cb_pipe_event)
{
    ...
    if (ctrl >= USBH_EHCI_TT_INSTANCES)
    {
        return ARM_DRIVER_ERROR;
    }

    usbh_ehci_reg_ptr[ctrl] = (HSUSBH_T *)((uint32_t)(usbh_ehci_ptr[ctrl])->ptr_EHCI);
...
static int32_t USBH0_HW_Initialize(ARM_USBH_SignalPortEvent_t cb_port_event, ARM_USBH_SignalPipeEvent_t cb_pipe_event)
{
    return USBH_HW_Initialize(0, cb_port_event, cb_pipe_event);
}
#if   (USBH_EHCI_TT_INSTANCES >= 2)
static int32_t USBH1_HW_Initialize(ARM_USBH_SignalPortEvent_t cb_port_event, ARM_USBH_SignalPipeEvent_t cb_pipe_event)
{
    return USBH_HW_Initialize(1, cb_port_event, cb_pipe_event);
}
#endif
...
static int32_t USBH_HW_Uninitialize(uint8_t ctrl)
{
    ...
    usbh_ehci_reg_ptr[ctrl]->UCMDR &= ~(HSUSBH_UCMDR_RUN_Msk);
...
static int32_t USBH0_HW_Uninitialize(void)
{
    return USBH_HW_Uninitialize(0);
}
#if   (USBH_EHCI_TT_INSTANCES >= 2)
static int32_t USBH1_HW_Uninitialize(void)
{
    return USBH_HW_Uninitialize(1);
}
#endif
...
static int32_t USBH_HW_PowerControl(uint8_t ctrl, ARM_POWER_STATE state)
{
    ...
    {
        case ARM_POWER_OFF:
            usbh_ehci_reg_ptr[ctrl]->UIENR = 0U;    // Disable all EHCI interrupts
            usbh_ehci_reg_ptr[ctrl]->UCMDR = 0U;    // Host Controller Stop

            // Wait for stop, max 1 sec
            tout = 10100;

            while ((usbh_ehci_reg_ptr[ctrl]->USTSR & (1UL << 12)) == 0U)   // While not halted
            {
                if (tout <= 100U)
                {
                    if (tout == 0U)
        ...
        case ARM_POWER_FULL:
            ...
            usbh_ehci_reg_ptr[ctrl]->UCMDR |= (1UL << 1);    // Host Controller Reset

            // Wait for reset to finish, max 1 sec
            tout = 10100;

            while ((usbh_ehci_reg_ptr[ctrl]->UCMDR & (1UL << 1)) != 0U)
            {
                if (tout <= 100U)
                {
                    if (tout == 0U)
            ...
            usbh_ehci_reg_ptr[ctrl]->UPSCR[0] = 0x22U;       // Controller Mode=Host Mode, VBPS Hi
            usbh_ehci_reg_ptr[ctrl]->UIENR   = HSUSBH_UIENR_USBIEN_Msk | HSUSBH_UIENR_UERRIEN_Msk | HSUSBH_UIENR_PCIEN_Msk; // Enable needed interrupts
            usbh_ehci_reg_ptr[ctrl]->UPFLBAR = (uint32_t)(usbh_ehci_ptr[ctrl])->ptr_PFL; // PFL

            //* enable port routing to EHCI           */
            usbh_ehci_reg_ptr[ctrl]->UCFGR |= HSUSBH_UCFGR_CF_Msk;

            // Wait for run to activate, max 1 sec
            usbh_ehci_reg_ptr[ctrl]->UCMDR = UCMDR_INT_THR_CTRL | HSUSBH_UCMDR_RUN_Msk; // Host Controller Run
            tout = 10100;

            while ((usbh_ehci_reg_ptr[ctrl]->USTSR & (1UL << 12)) != 0U)   // While halted
            {
                if (tout <= 100U)
                {
                    if (tout == 0U)
            ...
            //Pre Power on Port ??
            usbh_ehci_reg_ptr[ctrl]->UPSCR[0] |=  HSUSBH_UPSCR_PP_Msk;
...
static int32_t USBH0_HW_PowerControl(ARM_POWER_STATE state)
{
    return USBH_HW_PowerControl(0, state);
}
#if   (USBH_EHCI_TT_INSTANCES >= 2)
static int32_t USBH1_HW_PowerControl(ARM_POWER_STATE state)
{
    return USBH_HW_PowerControl(1, state);
}
#endif
...
static int32_t USBH_HW_PortVbusOnOff(uint8_t ctrl, uint8_t port, bool vbus)
{
    ...
    if (vbus)
    {
        usbh_ehci_reg_ptr[ctrl]->UPSCR[port] |=  HSUSBH_UPSCR_PP_Msk;
    }
    else
    {
        usbh_ehci_reg_ptr[ctrl]->UPSCR[port] &= ~HSUSBH_UPSCR_PP_Msk;
...
static int32_t USBH0_HW_PortVbusOnOff(uint8_t port, bool vbus)
{
    return USBH_HW_PortVbusOnOff(0, port, vbus);
}
#if   (USBH_EHCI_TT_INSTANCES >= 2)
static int32_t USBH1_HW_PortVbusOnOff(uint8_t port, bool vbus)
{
    return USBH_HW_PortVbusOnOff(1, port, vbus);
}
#endif
...
static int32_t USBH_HW_PortReset(uint8_t ctrl, uint8_t port)
{
    ...
    usbh_ehci_reg_ptr[ctrl]->UPSCR[port] &= ~HSUSBH_UPSCR_PE_Msk;    // Clear Port Enable
    usbh_ehci_reg_ptr[ctrl]->UPSCR[port] |=  HSUSBH_UPSCR_PRST_Msk;      // Port Reset

    tout = 5U;

    while ((usbh_ehci_reg_ptr[ctrl]->UPSCR[port] & HSUSBH_UPSCR_PRST_Msk) != 0U)
    {
        if (tout <= 5U)
        {
            if (tout == 0U)
    ...
    if (((usbh_ehci_reg_ptr[ctrl]->UPSCR[port] & HSUSBH_UPSCR_CCS_Msk) == 0) || ((usbh_ehci_reg_ptr[ctrl]->UPSCR[port] & HSUSBH_UPSCR_PE_Msk) == 0))
    {
        usbh_ehci_reg_ptr[ctrl]->UPSCR[port] |= HSUSBH_UPSCR_CSC_Msk;
        usbh_ehci_reg_ptr[ctrl]->UPSCR[port] |= HSUSBH_UPSCR_PO_Msk;
        return ARM_DRIVER_ERROR;
    }

    usbh_ehci_reg_ptr[ctrl]->UPSCR[port] |= HSUSBH_UPSCR_PEC_Msk;

    signal_port_event[ctrl](port, ARM_USBH_EVENT_RESET);

    return ARM_DRIVER_OK;
}
static int32_t USBH0_HW_PortReset(uint8_t port)
{
    return USBH_HW_PortReset(0, port);
}
#if   (USBH_EHCI_TT_INSTANCES >= 2)
static int32_t USBH1_HW_PortReset(uint8_t port)
{
    return USBH_HW_PortReset(1, port);
}
#endif
...
static int32_t USBH_HW_PortSuspend(uint8_t ctrl, uint8_t port)
{
    ...
    usbh_ehci_reg_ptr[ctrl]->UPSCR[port] |= HSUSBH_UPSCR_SUSPEND_Msk; // Set Suspend
    return ARM_DRIVER_OK;
}
static int32_t USBH0_HW_PortSuspend(uint8_t port)
{
    return USBH_HW_PortSuspend(0, port);
}
#if   (USBH_EHCI_TT_INSTANCES >= 2)
static int32_t USBH1_HW_PortSuspend(uint8_t port)
{
    return USBH_HW_PortSuspend(1, port);
}
#endif
...
static int32_t USBH0_HW_PortResume(uint8_t port)
{
    return USBH_HW_PortResume(0, port);
}
#if   (USBH_EHCI_TT_INSTANCES >= 2)
static int32_t USBH1_HW_PortResume(uint8_t port)
{
    return USBH_HW_PortResume(1, port);
}
#endif
...
static ARM_USBH_PORT_STATE USBH_HW_PortGetState(uint8_t ctrl, uint8_t port)
{
    ...
    PORTSC_val  = usbh_ehci_reg_ptr[ctrl]->UPSCR[port];

    if ((PORTSC_val & HSUSBH_UPSCR_CCS_Msk) != 0U)
    {
    ...
    if ((PORTSC_val & HSUSBH_UPSCR_OCA_Msk) != 0U)
    {
...
static ARM_USBH_PORT_STATE USBH0_HW_PortGetState(uint8_t port)
{
    return USBH_HW_PortGetState(0, port);
}
#if   (USBH_EHCI_TT_INSTANCES >= 2)
static ARM_USBH_PORT_STATE USBH1_HW_PortGetState(uint8_t port)
{
    return USBH_HW_PortGetState(1, port);
}
#endif
...
*/
static ARM_USBH_PIPE_HANDLE USBH_HW_PipeCreate(uint8_t ctrl, uint8_t dev_addr, uint8_t dev_speed, uint8_t hub_addr, uint8_t hub_port, uint8_t ep_addr, uint8_t ep_type, uint16_t ep_max_packet_size,
                                               uint8_t ep_interval)
{
    USBH_EHCI_qH        *ptr_qH;
    USBH_EHCI_qH        *ptr_first_qH;
    ...
    if (ep_type != ARM_USB_ENDPOINT_ISOCHRONOUS)  // If Control, Bulk or Interrupt pipe
    {
        ...
        // Get free qH and TI
        ptr_qH  = USBH_EHCI_qH_GetFree(ctrl);
        ...
        ptr_qTD = USBH_EHCI_qTD_GetFree(ctrl);
        ...
        ptr_TI  = USBH_EHCI_TI_GetFree(ctrl);
        ...
        switch (ep_type)                            // Endpoint Type
        {
            case ARM_USB_ENDPOINT_CONTROL:
            case ARM_USB_ENDPOINT_BULK:
                addr         = usbh_ehci_reg_ptr[ctrl]->UCALAR & ~0x1FU;
                ...
                if (!ptr_first_qH)                      // If this is first qH
                {
                    ...
                    ptr_qH->DW0.T            =  0U;       // T   = 0
                    usbh_ehci_reg_ptr[ctrl]->UCALAR = (uint32_t)ptr_qH & ~0x1FU;
                }
        ...
        if (((usbh_ehci_reg_ptr[ctrl]->UCMDR & (1UL << 5)) == 0U) && ((restart_flags & 1U) != 0U))
        {
            (void)USBH_EHCI_StartStop(ctrl, USBH_EHCI_ASYNC_SCHED,    true);
        }

        if (((usbh_ehci_reg_ptr[ctrl]->UCMDR & (1UL << 4)) == 0U) && ((restart_flags & 2U) != 0U))
        {
            (void)USBH_EHCI_StartStop(ctrl, USBH_EHCI_PERIODIC_SCHED, true);
        }
...
static ARM_USBH_PIPE_HANDLE USBH0_HW_PipeCreate(uint8_t dev_addr, uint8_t dev_speed, uint8_t hub_addr, uint8_t hub_port, uint8_t ep_addr, uint8_t ep_type, uint16_t ep_max_packet_size,
                                                uint8_t  ep_interval)
{
    return USBH_HW_PipeCreate(0, dev_addr, dev_speed, hub_addr, hub_port, ep_addr, ep_type, ep_max_packet_size, ep_interval);
}
#if   (USBH_EHCI_TT_INSTANCES >= 2)
static ARM_USBH_PIPE_HANDLE USBH1_HW_PipeCreate(uint8_t dev_addr, uint8_t dev_speed, uint8_t hub_addr, uint8_t hub_port, uint8_t ep_addr, uint8_t ep_type, uint16_t ep_max_packet_size,
                                                uint8_t  ep_interval)
{
    return USBH_HW_PipeCreate(1, dev_addr, dev_speed, hub_addr, hub_port, ep_addr, ep_type, ep_max_packet_size, ep_interval);
}
#endif
...
static int32_t USBH0_HW_PipeModify(ARM_USBH_PIPE_HANDLE pipe_hndl, uint8_t dev_addr, uint8_t dev_speed, uint8_t hub_addr, uint8_t hub_port, uint16_t ep_max_packet_size)
{
    return USBH_HW_PipeModify(0, pipe_hndl, dev_addr, dev_speed, hub_addr, hub_port, ep_max_packet_size);
}
#if   (USBH_EHCI_TT_INSTANCES >= 2)
static int32_t USBH1_HW_PipeModify(ARM_USBH_PIPE_HANDLE pipe_hndl, uint8_t dev_addr, uint8_t dev_speed, uint8_t hub_addr, uint8_t hub_port, uint16_t ep_max_packet_size)
{
    return USBH_HW_PipeModify(1, pipe_hndl, dev_addr, dev_speed, hub_addr, hub_port, ep_max_packet_size);
}
#endif
...
static int32_t USBH_HW_PipeTransferAbort(uint8_t ctrl, ARM_USBH_PIPE_HANDLE pipe_hndl)
{
    ...
    if (((USBH_EHCI_COMMON *)pipe_hndl)->DW0.Typ == USBH_EHCI_ELEMENT_TYPE_qH)
    {
        ...
        ptr_TI = USBH_EHCI_TI_Find_qH(ctrl, ptr_qH);
        ...
        if (ptr_TI->active == 0U)
        {
            return ARM_DRIVER_OK;    // If transfer is not active
        }
    ...
    else
    {
        ...
        ptr_TI = USBH_EHCI_TI_Find_PE(ctrl, (void *)pipe_hndl);
...
static int32_t USBH0_HW_PipeTransferAbort(ARM_USBH_PIPE_HANDLE pipe_hndl)
{
    return USBH_HW_PipeTransferAbort(0, pipe_hndl);
}
#if   (USBH_EHCI_TT_INSTANCES >= 2)
static int32_t USBH1_HW_PipeTransferAbort(ARM_USBH_PIPE_HANDLE pipe_hndl)
{
    return USBH_HW_PipeTransferAbort(1, pipe_hndl);
}
#endif
...
static int32_t USBH_HW_PipeDelete(uint8_t ctrl, ARM_USBH_PIPE_HANDLE pipe_hndl)
{
    ...
    if (((USBH_EHCI_COMMON *)pipe_hndl)->DW0.Typ == USBH_EHCI_ELEMENT_TYPE_qH)
    {
        ...
        else
        {
            ...
            if (((uint32_t)usbh_ehci_reg_ptr[ctrl]->UCALAR & ~0x1FU) == ((uint32_t)ptr_qH & ~0x1FU))
            {
                if (((uint32_t)ptr_qH->DW0.HorizLinkPtr << 5) == ((uint32_t)ptr_qH & ~0x1FU))   // If only 1
                {
                    usbh_ehci_reg_ptr[ctrl]->UCALAR = 0;
                }
                else
                {
                    // Point to next
                    usbh_ehci_reg_ptr[ctrl]->UCALAR = (uint32_t)ptr_qH->DW0.HorizLinkPtr << 5;
                    ptr_qH->DW1.H  = 1;
                    restart_flags |=  1U;
                }
            }
...
static int32_t USBH0_HW_PipeDelete(ARM_USBH_PIPE_HANDLE pipe_hndl)
{
    return USBH_HW_PipeDelete(0, pipe_hndl);
}
#if   (USBH_EHCI_TT_INSTANCES >= 2)
static int32_t USBH1_HW_PipeDelete(ARM_USBH_PIPE_HANDLE pipe_hndl)
{
    return USBH_HW_PipeDelete(1, pipe_hndl);
}
#endif
...
static int32_t USBH0_HW_PipeReset(ARM_USBH_PIPE_HANDLE pipe_hndl)
{
    return USBH_HW_PipeReset(0, pipe_hndl);
}
#if   (USBH_EHCI_TT_INSTANCES >= 2)
static int32_t USBH1_HW_PipeReset(ARM_USBH_PIPE_HANDLE pipe_hndl)
{
    return USBH_HW_PipeReset(1, pipe_hndl);
}
#endif
...
static int32_t USBH_HW_PipeTransfer(uint8_t ctrl, ARM_USBH_PIPE_HANDLE pipe_hndl, uint32_t packet, uint8_t *data, uint32_t num)
{
    ...
    ptr_TI   = USBH_EHCI_TI_Find_PE(ctrl, (USBH_EHCI_qH *)pipe_hndl);
    ...
    if (ptr_curr->DW0.Typ == USBH_EHCI_ELEMENT_TYPE_qH)      // If Control, Bulk or Interrupt pipe
    {
        ...
        ptr_qTD = (USBH_EHCI_qTD *)ptr_TI->ptr_transfer;
        ...
        if ((((USBH_EHCI_qH *)((uint32_t)ptr_TI->ptr_pipe))->DW2.ISM) == 0U)
        {
            for (j = 0U; j < 15U; j++)
            {
                ...
                {
                    usbh_ehci_reg_ptr[ctrl]->[j] |=  USBH_EHCI_PORTSC_PIC(1);   // Activate IND0
                }
            }
        }
...
static int32_t USBH0_HW_PipeTransfer(ARM_USBH_PIPE_HANDLE pipe_hndl, uint32_t packet, uint8_t *data, uint32_t num)
{
    return USBH_HW_PipeTransfer(0, pipe_hndl, packet, data, num);
}
#if   (USBH_EHCI_TT_INSTANCES >= 2)
static int32_t USBH1_HW_PipeTransfer(ARM_USBH_PIPE_HANDLE pipe_hndl, uint32_t packet, uint8_t *data, uint32_t num)
{
    return USBH_HW_PipeTransfer(1, pipe_hndl, packet, data, num);
}
#endif
...
static uint32_t USBH_HW_PipeTransferGetResult(uint8_t ctrl, ARM_USBH_PIPE_HANDLE pipe_hndl)
{
    ...
    ptr_TI = USBH_EHCI_TI_Find_qH(ctrl, (USBH_EHCI_qH *)pipe_hndl);
...
static uint32_t USBH0_HW_PipeTransferGetResult(ARM_USBH_PIPE_HANDLE pipe_hndl)
{
    return USBH_HW_PipeTransferGetResult(0, pipe_hndl);
}
#if   (USBH_EHCI_TT_INSTANCES >= 2)
static uint32_t USBH1_HW_PipeTransferGetResult(ARM_USBH_PIPE_HANDLE pipe_hndl)
{
    return USBH_HW_PipeTransferGetResult(1, pipe_hndl);
}
#endif
...
static uint16_t USBH_HW_GetFrameNumber(uint8_t ctrl)
{
    ...
    return ((uint16_t)(usbh_ehci_reg_ptr[ctrl]->UFINDR >> 3));
}
static uint16_t USBH0_HW_GetFrameNumber(void)
{
    return USBH_HW_GetFrameNumber(0);
}
#if   (USBH_EHCI_TT_INSTANCES >= 2)
static uint16_t USBH1_HW_GetFrameNumber(void)
{
    return USBH_HW_GetFrameNumber(1);
}
#endif
...
static void USBH_HW_IRQ_Handler(uint8_t ctrl)
{
    ...
    usbsts  = usbh_ehci_reg_ptr[ctrl]->USTSR;
    usbh_ehci_reg_ptr[ctrl]->USTSR = usbsts;
    usbintr = usbh_ehci_reg_ptr[ctrl]->UIENR;
    frindex = usbh_ehci_reg_ptr[ctrl]->UFINDR;

    // Analyze interrupt
    if ((usbsts & usbintr) != 0U)         // Check if not spurious Interrupt
    {
        if ((usbsts & HSUSBH_USTSR_PCD_Msk) != 0U)
        {
            // Port Change Detect
            for (i = 0U; i < USBH_EHCI_PORT; i ++)       // Go through all ports
            {
                if ((usbh_driver_capabilities[ctrl].port_mask & (1UL << i)) != 0U)
                {
                    // If port is enabled
                    portsc = usbh_ehci_reg_ptr[ctrl]->UPSCR[i];
                    usbh_ehci_reg_ptr[ctrl]->UPSCR[i] = portsc;

                    if ((portsc & HSUSBH_UPSCR_OCC_Msk) != 0U)
                    {
                        // Over-current Change
                        // Clear Connect Status Change
                        if ((portsc & HSUSBH_UPSCR_OCA_Msk) != 0U)
                        {
                    ...
                    if ((portsc & HSUSBH_UPSCR_CSC_Msk) != 0U)
                    {
                        // Connect Status Change
                        // Clear Connect Status Change
                        if ((portsc & HSUSBH_UPSCR_CCS_Msk) != 0U)
                        {
                            // Device Connected
                            port_act[ctrl] |= (1UL << i);
                    ...
                    if ((portsc & HSUSBH_UPSCR_PE_Msk) != 0U)
                    {
                        // Port Enable is set
                        if ((portsc & HSUSBH_UPSCR_FPR_Msk) != 0U)
                        {
                            // Port Remote Wake-up
                            //not suuprot..
        ...
        if ((usbsts & HSUSBH_USTSR_USBINT_Msk) != 0U)       // If USB Transaction Completed
        {
#if 1
                ...
                if (ptr_TI && ptr_TI->active)
                {
                            ...
                            ((status & (1UL << 6)) == 0U))                    // No Error
                    {
                        ...
                        if (!ep_event)
                        {
                            ...
                            (void)USBH_EHCI_qTD_Enqueue(ctrl, ptr_TI, (USBH_EHCI_qH *)((uint32_t)ptr_TI->ptr_pipe), (USBH_EHCI_qTD *)((uint32_t)ptr_TI->ptr_transfer), ptr_TI->packet, ptr_TI->data + ptr_TI->num_transferred_total,
                                                        ptr_TI->num_to_transfer);
                        }
                        else
                        {
                            // Transfer completed
...
static void Driver_USBH0_IRQ_Handler(void)
{
    USBH_HW_IRQ_Handler(0U);
}
#if   (USBH_EHCI_TT_INSTANCES >= 2)
static void Driver_USBH1_IRQ_Handler(void)
{
    USBH_HW_IRQ_Handler(1U);
}
#endif
