...
#define USBHn_OHCI_HCCA_SECTION_(x)     __attribute__((section(x)))
    ...
    #error "The HCCA area needs to be placed in the Non-Cacheable region,please.(.bss.NonCacheable.ZeroInit)"
#endif
...
static uint32_t _Ohci_Ctrl_Reg_Tampe[USBH_OHCI_INSTANCES];
...
static USBH_OHCI_ED *USBH_OHCI_ED_GetPrevious(uint8_t ctrl, USBH_OHCI_ED *ptr_ED)
{
    ...
    ptr_prev_ED = (USBH_OHCI_ED *)((uint32_t)(usbh_ohci_ptr[ctrl])->ptr_ED);
    ...
    while (cnt != 0U)
    {
        ...
        if (ptr_prev_ED->NextED == (uint32_t)ptr_ED)
        {
            return ptr_prev_ED;
        }
...
static USBH_OHCI_ED *USBH_OHCI_ED_Unlink(USBH_OHCI_ED *ptr_first_ED, USBH_OHCI_ED *ptr_ED)
{
    ...
    if ((ptr_first_ED_ == NULL) || (ptr_ED_ == NULL))
    {
...
static bool USBH_OHCI_ED_SetIntEntries(uint8_t ctrl, USBH_OHCI_ED *ptr_ED, uint8_t ep_speed, uint8_t ep_interval)
{
    ...
    if (USBH_OHCI_ED_ClearIntEntries(ctrl, ptr_ED) == false)
    {
        return false;
    }
...
static USBH_TransferInfo_t *USBH_OHCI_TI_GetFree(uint8_t ctrl)
{
    ...
    ptr_TI = (usbh_ohci_ptr[ctrl])->ptr_TI;
...
static USBH_TransferInfo_t *USBH_OHCI_TI_Find_TD(uint8_t ctrl, USBH_OHCI_TD *ptr_TD)
{
    ...
    while (cnt != 0U)
    {
        ...
        if (((uint32_t)ptr_TI->ptr_transfer & ~0x0FU) == (uint32_t)ptr_TD)
        {
            return ptr_TI;
        }
...
static USBH_TransferInfo_t *USBH_OHCI_TI_Find_ED(uint8_t ctrl, USBH_OHCI_ED *ptr_ED)
{
    ...
    while (cnt != 0U)
    {
        ...
        if ((uint32_t)ptr_TI->ptr_pipe == (uint32_t)ptr_ED)
        {
            return ptr_TI;
        }
...
static int32_t USBH_HW_PowerControl(uint8_t ctrl, ARM_POWER_STATE state)
{
    ...
    switch (state)
    {
        case ARM_POWER_OFF:
            ...
            if (ctrl == 0)
            {
                usbh_ohci_reg_ptr[ctrl]->HcRhDescriptorA   = 0x02000001;
            }
            else
            {
                usbh_ohci_reg_ptr[ctrl]->HcRhDescriptorA   = 0x01000901;
            }

            usbh_ohci_reg_ptr[ctrl]->HcRhDescriptorB   &= ~0xFFFFFFFFU;
        ...
        case ARM_POWER_FULL:
            ...
            usbh_ohci_reg_ptr[ctrl]->HcInterruptDisable = USBH_OHCI_HcInterruptEnable_MIE;
...
static int32_t USBH_HW_PortSuspend(uint8_t ctrl, uint8_t port)
{
    ...
    {
        //
        _Ohci_Ctrl_Reg_Tampe[ctrl] = usbh_ohci_reg_ptr[ctrl]->HcControl;
        usbh_ohci_reg_ptr[ctrl]->HcControl = (_Ohci_Ctrl_Reg_Tampe[ctrl] & ~USBH_OHCI_HcControl_CBSR) | USBH_OHCI_FS_USB_SUSPEND;
    }
    else if (usbh_ohci_ptr[ctrl]->ctrl == USBH1_OHCI_DRV_NUM)
    {
        //
        _Ohci_Ctrl_Reg_Tampe[ctrl] = usbh_ohci_reg_ptr[ctrl]->HcControl;
        usbh_ohci_reg_ptr[ctrl]->HcControl = (_Ohci_Ctrl_Reg_Tampe[ctrl] & ~USBH_OHCI_HcControl_CBSR) | USBH_OHCI_FS_USB_SUSPEND;
...
static ARM_USBH_PORT_STATE USBH_HW_PortGetState(uint8_t ctrl, uint8_t port)
{
    ...
    if ((usbh_ohci_reg_ptr[ctrl]->HcRhPortStatus[port] & USBH_OHCI_HcRhPortStatus_CCS) != 0U)
    {
    ...
    if ((usbh_ohci_reg_ptr[ctrl]->HcRhPortStatus[port] & USBH_OHCI_HcRhPortStatus_POCI) != 0U)
    {
    ...
    if ((usbh_ohci_reg_ptr[ctrl]->HcRhPortStatus[port] & USBH_OHCI_HcRhPortStatus_LSDA) != 0U)
    {
                                               ...
                                               uint8_t ep_interval)
{
    ...
    // Get free ED and TI
    ptr_ED = USBH_OHCI_ED_GetFree(ctrl);
    ...
    ptr_TI = USBH_OHCI_TI_GetFree(ctrl);
...
static int32_t USBH_HW_PipeTransferAbort(uint8_t ctrl, ARM_USBH_PIPE_HANDLE pipe_hndl)
{
    ...
    ptr_TI = USBH_OHCI_TI_Find_ED(ctrl, ptr_ED);
...
static int32_t USBH_HW_PipeTransfer(uint8_t ctrl, ARM_USBH_PIPE_HANDLE pipe_hndl, uint32_t packet, uint8_t *data, uint32_t num)
{
    ...
    ptr_TD = USBH_OHCI_TD_GetFree(ctrl);
    ...
    ptr_TI = USBH_OHCI_TI_Find_ED(ctrl, (USBH_OHCI_ED *)pipe_hndl);
...
static void USBH_HW_IRQ_Handler(uint8_t ctrl)
{
    ...
    if ((int_stat & int_en) != 0U)        // Check if not spurious Interrupt
    {
        ...
        if ((int_stat & USBH_OHCI_HcInterruptStatus_WDH) != 0U)
        {
            ...
            ptr_TD = (USBH_OHCI_TD *)(hcca_done_head & (~1U));
                ...
                if ((ptr_TI != NULL) && (ptr_TI->active != 0U))
                {
                    ...
                    if (!cc)
                    {
                        ...
                        if (!ep_event)
                        {
                            ...
                            (void)USBH_OHCI_TD_Enqueue(ctrl, (USBH_OHCI_ED *)((uint32_t)ptr_TI->ptr_pipe), (USBH_OHCI_TD *)((uint32_t)ptr_TI->ptr_transfer), ptr_TI->packet, ptr_TI->data + ptr_TI->num_transferred_total,
